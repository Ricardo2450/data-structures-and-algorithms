...
const toLastNames = people => {
  // Solution code here...
return people.map(fullName => `${fullName.firstName} ${fullName.lastName}`);
};
...
const addValues = (arr) => {
  // Solution code here...
  let newArr = 0;
  return arr.reduce((all, num) => all + num, newArr);
};
...
const reversedString = (str) => {
  ...
  return newArr.reduce((acc, currentLetter) => currentLetter + acc, '');
};

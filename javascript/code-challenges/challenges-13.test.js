...
// Solution code here...
let longestStr = arr.reduce((longest, str)=> longest.length > str.length ? longest : str,'');
  return arr.findIndex(string => string === longestStr);
};
...
const allHappy = (arr) => {
  // Solution code here...
  let regex = /[:][)]/;
  return arr.map(str=> regex.test(str)).includes(false) ? false : true;
};
